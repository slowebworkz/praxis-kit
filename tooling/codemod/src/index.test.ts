...
describe('renameInProject — cascades to every reference, not just call sites', () => {
  it('renames a bare reference and a method call off the binding', () => {
    // Unaliased: nameNode.rename() is a real identifier rename, so it reaches every reference
    // to the binding project-wide — not only the import line or JSX/factory call sites.
    const project = makeProject({
      'test.ts': [
        "import { createPolymorphicComponent } from '@praxis-kit/react'",
        'const fn = createPolymorphicComponent',
        'const bound = createPolymorphicComponent.bind(null)',
      ].join('\n'),
    })
    ...
    expect(text(project, 'test.ts')).toMatchInlineSnapshot(`
      "import { createContractComponent } from '@praxis-kit/react'
      const fn = createContractComponent
      const bound = createContractComponent.bind(null)"
    `)
  })
})

describe('renameInProject — preserves a trailing comment on the specifier', () => {
  it('renames the identifier without disturbing the comment', () => {
    const project = makeProject({
      'test.ts': [
        'import {',
        '  createPolymorphicComponent, // comment',
        "} from '@praxis-kit/react'",
        "const Button = createPolymorphicComponent({ tag: 'button' })",
      ].join('\n'),
    })
    ...
    expect(result).toContain('createContractComponent, // comment')
    expect(result).toContain("const Button = createContractComponent({ tag: 'button' })")
  })
})

...
export function designTokensPlugin(options?: DesignTokensOptions): Plugin {
  ...
  return {
    ...
    // The manifest is rebuilt from scratch each build — otherwise a watch-mode rebuild after a
    // file is deleted or renamed keeps the removed component's classes in the emitted JSON.
    buildStart() {
      accumulated.clear()
    },

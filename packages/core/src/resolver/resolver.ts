...
/**
 * A lightweight resolve loop: tag → `allowedAs` guard → prop merge → built-in ARIA policy →
 * classes. It takes the minimal {@link ResolverOptions} (no `variants`, no `enforcement`), so it
 * runs **only the AriaPolicyEngine's built-in rule tiers** — a component with custom
 * `enforcement.aria` / `enforcement.rules` or variant-aware validation goes through
 * `createPolymorphic` instead, whose `createAriaPipeline` threads `ariaRules` + `variantKeys`
 * into the engine.
 *
 * `allowedAs` constrains a caller-supplied `as` override only — a component's own `defaultTag`

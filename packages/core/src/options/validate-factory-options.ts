...
>(resolved: ResolvedFactoryOptions<TDefault, Props, V, TPreset>, diagnostics: Diagnostics): void {
  // `../pk`'s `diagnostics.active` (warn-scoped gate) is `warnActive` here — see the same rename
  // in `lib/diagnostics` / `InvariantBase`. This validator only emits warning-level config
  // diagnostics, so the scope matches.

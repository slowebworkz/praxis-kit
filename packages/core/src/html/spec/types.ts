...
export function resolveAllowedRoles(
  ...
  switch (policy.kind) {
    case 'fixed':
      return policy.roles
    case 'byProp': {
      // An unrecognized discriminator value resolves to the fallback, matching HTML's own
      // "unknown `<input type>` behaves as `text`" rule — so `<input type="madeup" role="menu">`
      // is checked against the same allowed set as `<input type="text">`, not skipped.

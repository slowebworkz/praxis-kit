...
describe('requiredReadOnlyConflictRule', () => {
  ...
  it('flags the HTML boolean-attribute forms (required="" readOnly=""), not just booleans', () => {
    const [result] = requiredReadOnlyConflictRule(ctx({ required: '', readOnly: 'readonly' }))
    expect(result).toMatchObject({ valid: false })
  })

  it('does not flag an explicit required={false}', () => {
    expect(requiredReadOnlyConflictRule(ctx({ required: false, readOnly: true }))).toEqual([])
  })
})

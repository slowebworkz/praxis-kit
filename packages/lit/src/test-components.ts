...
// ─── Nav (ARIA exercise) ─────────────────────────────────────────────────────
// Exercises the resolveAria path — a <nav> with an explicit aria-label.
...
function isTabsTrigger(child: unknown): child is Element {
  return child instanceof Element && child.tagName.toLowerCase() === 'praxis-tabs-trigger'
}

function isTabsContent(child: unknown): child is Element {
  return child instanceof Element && child.tagName.toLowerCase() === 'praxis-tabs-content'
}

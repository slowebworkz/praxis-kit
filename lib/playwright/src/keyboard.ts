 ...
 *
 * `count` must be a non-negative integer — a fractional or negative value is almost always a
 * test bug, so it throws rather than silently rounding (`1.5` → tabs twice) or no-opping (`-1`).
 */
export async function tabTimes(page: Page, count: number): Promise<void> {
  if (!Number.isInteger(count) || count < 0) {
    throw new RangeError(`tabTimes: count must be a non-negative integer, got ${count}`)
  }

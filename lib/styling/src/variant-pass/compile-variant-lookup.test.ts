...
describe('compileVariantLookup', () => {
  ...
  it('honors array-valued compound conditions (size: ["sm", "lg"]) — same as CVA / diagnose', () => {
    const config: VariantConfig = {
      variants: {
        size: { sm: 'text-sm', md: 'text-md', lg: 'text-lg' },
        color: { red: 'text-red-500' },
      },
      compounds: [{ size: ['sm', 'lg'], color: 'red', class: 'font-bold' }],
    }
    const table = compileVariantLookup(config)
    expect(table).not.toBeNull()
    expect(table!['color:red|size:sm']).toContain('font-bold')
    expect(table!['color:red|size:lg']).toContain('font-bold')
    expect(table!['color:red|size:md']).not.toContain('font-bold')
  })

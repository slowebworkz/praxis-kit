...
// `../pk` imported this from `@praxis-kit/pipeline` as a shared alias; the rewritten `lib/pipeline`
// no longer carries it, and this module is its only consumer here. A flat string→string map of
// explicitly-set variant props (and, in the compiled table, precomputed class strings).
...
// A compound condition value may be a single string or a list of accepted strings
// (`size: ['sm', 'lg']`) — same contract `diagnoseClassPipeline` honors. The compiled path must
// match it identically or the precomputed table diverges from runtime CVA output.
function conditionMatches(got: string | undefined, expected: string | readonly string[]): boolean {
...
function matchesCompound(compound: CompoundVariant, effective: DefaultMap): boolean {
  ...
  iterate.forEachEntry(compound, (key, value) => {
    if (key === 'class') return
    if (!conditionMatches(effective[key], value)) {
      matches = false
    }
  })

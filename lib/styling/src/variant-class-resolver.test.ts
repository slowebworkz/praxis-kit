...
describe('VariantClassResolver — recipeMap', () => {
  ...
  it('treats every string recipe key uniformly — "" is a key, not "no recipe"', () => {
    const r = new VariantClassResolver(sizeFn, { '': { size: 'lg' } })
    // recipe: '' looks up recipeMap[''] and merges its preset, same as any other key.
    expect(r.resolve({ props: {}, recipe: '' })).toContain('text-lg')
    // recipe: undefined is the only "no recipe" — falls to defaultVariants.
    expect(r.resolve({ props: {}, recipe: undefined })).toContain('text-sm')
  })
})

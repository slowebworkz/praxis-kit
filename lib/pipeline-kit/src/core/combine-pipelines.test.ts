...
describe('anyPipeline()', () => {
  it('returns the first defined result', () => {
    const first: Pipeline<[string], number | undefined> = () => {}
  ...
  it('fully specifies evaluation order: earlier pipelines run, later ones do not', () => {
    const first = vi.fn<Pipeline<[string], number | undefined>>(() => {})
  ...
  it('returns undefined when every pipeline returns undefined', () => {
    const first: Pipeline<[string], number | undefined> = () => {}
    const second: Pipeline<[string], number | undefined> = () => {}

    const combined = anyPipeline([first, second])
    expect(combined('anything')).toBeUndefined()
  })

  it('treats falsy values other than undefined as successful results (undefined is the only miss)', () => {
    for (const value of [0, false, '', null] as const) {
      const hit: Pipeline<[], typeof value | undefined> = () => value
      const later: Pipeline<[], typeof value | undefined> = () => value
      const miss: Pipeline<[], typeof value | undefined> = () => {}
      const combined = anyPipeline<[], typeof value>([miss, hit, later])

...
describe('ChildrenEvaluator constructor', () => {
  ...
  it('still validates a contradictory *static* positional rule when a dynamic rule is also present', () => {
    expect(
      () =>
        new ChildrenEvaluator(
          [
            {
              name: 'header',
              match: (c): c is Header => c instanceof Header,
              position: 'first',
              cardinality: { max: 2 }, // contradictory — static
            },
            {
              name: 'body',
              match: (c): c is Body => c instanceof Body,
              cardinality: dynamic(() => ({ max: 3 })), // forces the dynamic-rules branch
            },
          ],
          throwDiagnostics,
          'Test',

...
// ---------------------------------------------------------------------------
// Positional cardinality invariant — position="first"|"last" implies max=1
// ---------------------------------------------------------------------------
...
describe('normalizeChildRule() — positional cardinality invariant', () => {
  ...
  it('throws for position="last" with an explicit unbounded max', () => {
    expect(() =>
      normalizeChildRule({
        name: 'footer',
        match: matchAll,
  ...
  it('allows position="first" with max=1 (the implied value, stated explicitly)', () => {
    const n = normalizeChildRule({
      name: 'header',
      match: matchAll,
      position: 'first',

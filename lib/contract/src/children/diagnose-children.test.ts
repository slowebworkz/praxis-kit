...
describe('diagnoseChildren', () => {
  ...
  // Parity with ChildrenEvaluator: both normalize through normalizeChildRule, so a
  // structurally-impossible positional rule is rejected the same way by both APIs.
  it('throws RangeError for a contradictory positional rule (position="first" + max>1)', () => {
    const bad: ChildRuleInput = {
      name: 'header',
      match: (c) => c instanceof Header,
      position: 'first',
      cardinality: { max: 2 },
    }
    expect(() => diagnoseChildren([bad], [headerEl])).toThrow(RangeError)
  })
})

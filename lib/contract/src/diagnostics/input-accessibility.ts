...
export const InputAccessibilityDiagnostics = {
  ...
  requiredReadOnlyConflict(): Fact {
    return accessibilityFact({
      ...
      severity: 'warning',
      message:
        '"required" has no effect while this field is readOnly — the two attributes together usually signal an unintended state.',
      rationale:
        'The combination is valid HTML, but a readOnly control cannot be edited, so its "required" constraint can never be satisfied or violated through user interaction. Consider using disabled instead of readOnly, or only applying required when the field is editable.',
    })
  },
}
